...
public class ServletPesquisaPessoa extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String nis = request.getParameter("nis").trim();

		HttpSession session = request.getSession();
		
		if (!nome.equals("")) { 
			// Pesquisa por nome do usuario. 
			List<Pessoa> pessoas = Facade.getInstance().getControlePessoa().ControleBuscaProfessorPorNome(nome);
			session.setAttribute("pessoas", pessoas);
		} else if (!cpf.equals("")) {
			// Busca por código do cpf. 
			List<Pessoa> pessoas = Facade.getInstance().getControlePessoa().ControleListaPessoaPorCpf(cpf);
			session.setAttribute("pessoas", pessoas);
		} else if (nome.equals("") && cpf.equals("")) {
			// Busca todos alunos por idescola. 
			List<Pessoa> pessoas = Facade.getInstance().getControlePessoa().ControleListaPessoa();
			session.setAttribute("pessoas", pessoas);
		}

		RequestDispatcher dispatcher = request.getRequestDispatcher("/pesquisarpessoa.jsp");
